...
public class RollatorAverageSpeedParser extends FootAverageSpeedParser {
    private final int maxInclinePercent = 6;

    public RollatorAverageSpeedParser(EncodedValueLookup lookup, PMap properties) {
        this(lookup.getDecimalEncodedValue(properties.getString("name", VehicleSpeed.key("rollator"))));
    }

    protected RollatorAverageSpeedParser(DecimalEncodedValue speedEnc) {
        super(speedEnc);
