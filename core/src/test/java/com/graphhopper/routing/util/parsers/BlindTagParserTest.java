 ...
 * @author Peter Karich
 */
public class BlindTagParserTest {
    private final BooleanEncodedValue blindAccessEnc = VehicleAccess.create("blind");
    private final DecimalEncodedValue blindAvgSpeedEnc = VehicleSpeed.create("blind", 4, 1, false);
    private final DecimalEncodedValue blindPriorityEnc = VehiclePriority.create("blind", 4, PriorityCode.getFactor(1), false);
    ...
    private final EncodingManager encodingManager = EncodingManager.start()
            .add(blindAccessEnc).add(blindAvgSpeedEnc).add(blindPriorityEnc).add(RouteNetwork.create(FootNetwork.KEY))
            .add(bikeAccessEnc).add(bikeAvgSpeedEnc).add(RouteNetwork.create(BikeNetwork.KEY))
            .add(carAccessEnc).add(carAvSpeedEnc)
            .build();
    private final BlindAccessParser accessParser = new BlindAccessParser(encodingManager, new PMap());
    private final BlindAverageSpeedParser speedParser = new BlindAverageSpeedParser(encodingManager, new PMap());
    private final BlindPriorityParser prioParser = new BlindPriorityParser(encodingManager, new PMap());

    public BlindTagParserTest() {
    ...
    public void testGetSpeed() {
        ...
        int edgeId = 0;
        blindAccessEnc.setBool(false, edgeId, edgeIntAccess, true);
        blindAccessEnc.setBool(true, edgeId, edgeIntAccess, true);
        blindAvgSpeedEnc.setDecimal(false, edgeId, edgeIntAccess, 10);
        assertEquals(10, blindAvgSpeedEnc.getDecimal(false, edgeId, edgeIntAccess), 1e-1);
    }
    ...
    public void testSteps() {
        ...
        assertEquals(MEAN_SPEED, blindAvgSpeedEnc.getDecimal(false, edgeId, edgeIntAccess), 1e-1);

        way.setTag("highway", "steps");
        edgeIntAccess = new ArrayEdgeIntAccess(encodingManager.getIntsForFlags());
        speedParser.handleWayTags(edgeId, edgeIntAccess, way);
        assertTrue(MEAN_SPEED > blindAvgSpeedEnc.getDecimal(false, edgeId, edgeIntAccess));
    }
    ...
    public void testCombined() {
        ...
        edge.set(blindAvgSpeedEnc, 10.0).set(blindAccessEnc, true, true);
        edge.set(carAvSpeedEnc, 100.0).set(carAccessEnc, true, false);

        assertEquals(10, edge.get(blindAvgSpeedEnc), 1e-1);
        assertTrue(edge.get(blindAccessEnc));
        assertTrue(edge.getReverse(blindAccessEnc));
    ...
    public void testGraph() {
        BaseGraph g = new BaseGraph.Builder(encodingManager).create();
        g.edge(0, 1).setDistance(10).set(blindAvgSpeedEnc, 10.0).set(blindAccessEnc, true, true);
        g.edge(0, 2).setDistance(10).set(blindAvgSpeedEnc, 5.0).set(blindAccessEnc, true, true);
        g.edge(1, 3).setDistance(10).set(blindAvgSpeedEnc, 10.0).set(blindAccessEnc, true, true);
        EdgeExplorer out = g.createEdgeExplorer(AccessFilter.outEdges(blindAccessEnc));
    ...
    public void testRailPlatformIssue366() {
        ...
        speedParser.handleWayTags(edgeId, edgeIntAccess, way);
        assertTrue(blindAccessEnc.getBool(false, edgeId, edgeIntAccess));
        assertEquals(5, blindAvgSpeedEnc.getDecimal(false, edgeId, edgeIntAccess));
        ...
        accessParser.handleWayTags(edgeId, edgeIntAccess, way);
        speedParser.handleWayTags(edgeId, edgeIntAccess, way);
        assertFalse(blindAccessEnc.getBool(false, edgeId, edgeIntAccess));
        assertEquals(0, blindAvgSpeedEnc.getDecimal(false, edgeId, edgeIntAccess));
    }
    ...
    public void testOneway() {
        ...
        assertTrue(blindAccessEnc.getBool(false, edgeId, edgeIntAccess));
        assertFalse(blindAccessEnc.getBool(true, edgeId, edgeIntAccess));
        ...
        accessParser.handleWayTags(edgeId, edgeIntAccess, way);
        assertFalse(blindAccessEnc.getBool(false, edgeId, edgeIntAccess));
        assertTrue(blindAccessEnc.getBool(true, edgeId, edgeIntAccess));
    }
    ...
    public void testMixSpeedAndSafe() {
        ...
        assertTrue(blindAccessEnc.getBool(false, edgeId, edgeIntAccess));
        assertTrue(blindAccessEnc.getBool(true, edgeId, edgeIntAccess));
        assertEquals(5, blindAvgSpeedEnc.getDecimal(false, edgeId, edgeIntAccess), 1e-1);
    ...
    public void testSlowHiking() {
        ...
        edgeIntAccess = new ArrayEdgeIntAccess(encodingManager.getIntsForFlags());
        speedParser.handleWayTags(edgeId, edgeIntAccess, way);
        assertEquals(SLOW_SPEED, blindAvgSpeedEnc.getDecimal(false, edgeId, edgeIntAccess), 1e-1);
    }
    ...
    public void testFord() {
        ...
        BlindAccessParser blockFordsParser = new BlindAccessParser(encodingManager, new PMap("block_fords=true"));
        node = new ReaderNode(1, -1, -1);
        node.setTag("ford", "no");
